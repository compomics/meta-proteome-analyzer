...
public class ECReader {
	// A EC-entry
//	private static ECEntry ecEntry;
	
	/**
	 * Method to read Map with EC-entries
	 * @param filePath
	 * @return
	 */
	public static Map<String, ECEntry> readEC(String filePath) {
			...
			try {
				...
				for (int i = 0; i < level1NodeList.getLength(); i++) {
					if (level1NodeList.item(i).getNodeName() == "ec_class") {
						...
						ecNumberLevel1 = level1Node.getAttributes().item(0).getTextContent();
						
						if (level1Node.getChildNodes().item(1).getNodeName()== "name") {
							ecNameLevel1   = level1Node.getChildNodes().item(1).getTextContent();
						}
						if (level1Node.getChildNodes().item(3).getNodeName()== "description") {
							ecDescLevel1   = level1Node.getChildNodes().item(3).getTextContent();
						}
						// Fill  maps
						ecEntry = new ECEntry();
						...
						for (int j = 0; j < level2NodeList.getLength(); j++) { // First entries are name and description
							if (level2NodeList.item(j).getNodeName() == "ec_subclass") {
								Node level2Node = level2NodeList.item(j);

								// Get EC: entries
								ecNumberLevel2 = ecNumberLevel1 + "." + level2Node.getAttributes().item(0).getTextContent();
								if (level2Node.getChildNodes().item(1).getNodeName() == "name") {
									ecNameLevel2   = level2Node.getChildNodes().item(1).getTextContent();
								}
								if (level2Node.getChildNodes().item(3).getNodeName() == "description") {
									ecDescLevel2   = level2Node.getChildNodes().item(3).getTextContent();
								}
								...
								for (int k = 0; k < level3NodeList.getLength(); k++) { // First entries are name and description
									if (level3NodeList.item(k).getNodeName() == "ec_sub-subclass") {
										Node level3Node = level3NodeList.item(k);

										// Get EC-entries
										ecNumberLevel3 = ecNumberLevel2 + "." + level3Node.getAttributes().item(0).getTextContent();
										if (level3Node.getChildNodes().item(1).getNodeName() == "name"){
											ecNameLevel3   = level3Node.getChildNodes().item(1).getTextContent();
										}
										...
										for (int m = 0; m < level4NodeList.getLength(); m++) {
											if (level4NodeList.item(m).getNodeName() == "enzyme") {

												// Get EC-entries
												Node level4Node = level4NodeList.item(m);
												ecNumberLevel4 = ecNumberLevel3 + "." +  level4Node.getAttributes().item(0).getTextContent();
												// Control EC number ( changed by transfers or deletes
												if (ecNumberLevel4 != level4Node.getChildNodes().item(1).getTextContent().substring(3)) {
													// Delete EC_ at the beginnnig
													ecNumberLevel4 = level4Node.getChildNodes().item(1).getTextContent().substring(3);
												}
												
												if (level4Node.getChildNodes().item(3).getNodeName()== "accepted_name") {
													ecNameLevel4 = level4Node.getChildNodes().item(3).getTextContent();
												}else{
													ecNameLevel4 = level4Node.getChildNodes().item(3).getNodeName() + " " + level4Node.getChildNodes().item(3).getFirstChild().getNextSibling().getTextContent();
												}
												
												try {// Try  because sometimes no comments
													if (level4Node.getChildNodes().item(5).getNodeName()== "comments") {
														ecDescLevel4 = level4Node.getChildNodes().item(5).getChildNodes().item(1).getTextContent();
													}
												} catch (Exception e) {
												}
