...
public class ComparePanel extends JPanel{
	 ...
	 * Level of comparison.
	 */
	private static final String[] ENTRIES = new String[] {"Accessions", "Metaproteins"};
	 ...
	 */
	private static final String[] GROUPING = new String[] {"Average", "Max", "Min"};
	...
	public ComparePanel() {
		...
		// Initialize ProjectManager to access to projects and experiments in the database
		try {
	...
	private void initComponents() {
		this.setLayout(new FormLayout("5dlu,p:g, 5dlu", "5dlu, p, 5dlu, p, 5dlu, p:g, 5dlu")); // Column, Row
		...
		samplesPnl.setLayout(new FormLayout("5dlu,p:g, 5dlu, p:g, 5dlu", "5dlu, p, 5dlu")); // Column, Row
		...
		samplesTtlPnl.setTitleForeground(ttlForeground);
			...
			groupPnl.setLayout(new FormLayout(" 5dlu, p:g, 5dlu,p:g, 5dlu", "5dlu, p, 5dlu, p, 5dlu")); // Column, Row
			...
			addGroupBtn.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent arg0) {
					String newGroup = (String)JOptionPane.showInputDialog("Enter some text : ", "" );
					if (newGroup != null) {
			...
			measurementPnl.setLayout(new FormLayout(" 5dlu, p:g, 5dlu,p:g, 5dlu", "5dlu, p, 5dlu, p, 5dlu")); // Column, Row
			...
			addMeasurementBtn.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					...
					// first get group
					String key = (String) groupTbl.getValueAt(groupTbl.getSelectedRow(), 1);
					System.out.println(groupMap.get(key));
					// get via Dialog the experiments
					JPanel chooseExpPnl 	= new JPanel();
					chooseExpPnl.setLayout(new FormLayout("5dlu, p, 5dlu, p, 5dlu" , "5dlu, p, 5dlu"));
					...
					projectTbl.getSelectionModel().addListSelectionListener(new ListSelectionListener() {
						...
						public void valueChanged(ListSelectionEvent e) {
							...
							try {
								List<Experiment> projectExperiments = projectManager.getProjectExperiments(projectID);
								int index = 1;
							for (int i = 0; i < projectExperiments.size(); i++) {
								String title = projectExperiments.get(i).getTitle();
								((DefaultTableModel) experimentTable.getModel()).addRow(new Object [] {index++,title});
							}
					...
					int ret = JOptionPane.showConfirmDialog(clientFrame, chooseExpPnl, "Choose an Experiment", JOptionPane.OK_CANCEL_OPTION);
				...
				private void refreshProjectsTable() {
					...
					for (int i = 0; i < projects.size(); i++) {
						...
						((DefaultTableModel) projectTbl.getModel()).addRow(new Object [] {index++,title});
					}
...
// Write method to fill the table
	...
	private void refreshGroupTable() {
		...
		for (Entry<String, List<Experiment>> entry : groupMap.entrySet()) {
			((DefaultTableModel) groupTbl.getModel()).addRow(new Object [] {index++,entry.getKey()});
		}
