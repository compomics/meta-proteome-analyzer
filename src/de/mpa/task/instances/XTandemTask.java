 ...
 *
 */
public class XTandemTask extends Task {	
	...
	public XTandemTask(File mgfFile, DbSearchSettings searchSettings) {
		...
		this.params = searchSettings.getXTandemParams();
		// Add units for fragments and precursors
		if(searchSettings.isPrecIonTolPpm()){ 
		...
		if (searchType == SearchType.TARGET) {
	        ...
	        if (params == null) {
	        	XTandemParameters defaultParams = new XTandemParameters();
	        	defaultParams.put("refine", new BooleanParameter(false, "Use first-pass search (refinement)", "Enable first-pass search (refinement)", "Refinement"));
