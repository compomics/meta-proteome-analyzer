...
public class CypherQueryFactory {
	...
	public static CypherQuery getProteinsByDescriptionPattern(String pattern, boolean not) {
		...
		conditions.add(
				new CypherCondition("proteins." + ProteinProperty.DESCRIPTION, pattern, CypherOperatorType.REGEXP));
		// List<CypherCondition> conditions = null;
	 ...
	 * 
	 * @param type
	 *            RelationType for specific ontology
	 * @return CypherQuery for molecular function ontologies grouped by
	 *         proteins.
	 */
	...
	public static CypherQuery getBiologicalProcessesWithCountsByExperiments(String countIdentifier) {
		return new CypherQuery("START ontologies = node:Ontologies(\"Identifier:*\")"
					+ "MATCH (ontologies)<-[:INVOLVED_IN_BIOPROCESS]-(proteins)-[:HAS_PEPTIDE]->"
					+ "(peptides)<-[:IS_MATCH_IN]-(psms)-[:BELONGS_TO_EXPERIMENT]->(experiments), "
					+ "(experiments)<-[:BELONGS_TO_EXPERIMENT]-(proteins)" + "RETURN experiments, count(distinct "
					+ countIdentifier + "), ontologies");
	}
	 ...
	 *            Taxonomic rank.
	 * @param depth
	 *            Depth of the taxonomy (e.g. lowest level == 1 --> species).
	 * @return CypherQuery instance.
	 */
	public static CypherQuery getTaxonomyWithCountsByExperiments(String countIdentifier, String rank) {
		return new CypherQuery("START parent = node:Taxa(\"Identifier:*\")"
				...
				+ "WHERE (parent.Rank = '" + rank + "')" + "RETURN experiments, count(distinct " + countIdentifier
				+ "), parent");
	}
	...
	public static CypherQuery getSubspeciesWithCountsByExperiments(String countIdentifier) {
		return new CypherQuery("START taxa = node:Taxa(\"Identifier:*\")"
				+ "MATCH (taxa)<-[:BELONGS_TO_TAXONOMY]-(proteins)-[:HAS_PEPTIDE]->(peptides)"
				+ "<-[:IS_MATCH_IN]-(psms)-[:BELONGS_TO_EXPERIMENT]"
				+ "->(experiments), (experiments)<-[:BELONGS_TO_EXPERIMENT]-(proteins)"
				+ "WHERE (taxa.Rank = 'Subspecies')" + "RETURN experiments, count(distinct " + countIdentifier
				+ "), taxa");
	}
