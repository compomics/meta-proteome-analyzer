...
public class PropertyTableAccessor implements Deleteable, Retrievable, Updateable, Persistable {
	...
	public int update(Connection aConn) throws SQLException {
		...
		PreparedStatement lStat = aConn.prepareStatement("UPDATE property SET propertyid = ?, fk_projectid = ?, name = ?, value = ? WHERE propertyid = ?");
		lStat.setLong(1, iPropertyid);
		lStat.setLong(2, iFk_projectid);
		lStat.setObject(3, iName);
		lStat.setObject(4, iValue);
		lStat.setLong(5, iPropertyid);
		int result = lStat.executeUpdate();
		lStat.close();
		this.iUpdated = false;
		return result;
	...
	public int persist(Connection aConn) throws SQLException {
		PreparedStatement lStat = aConn.prepareStatement("INSERT INTO property (propertyid, fk_projectid, name, value) values(?, ?, ?, ?)");
