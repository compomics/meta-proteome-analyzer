...
public class ProjectTableAccessor implements Deleteable, Retrievable, Updateable, Persistable {
	 ...
	 */
	public ProjectTableAccessor(HashMap<Object, Object> aParams) {
	 ...
	 */
	public static ProjectTableAccessor findFromTitle(String title, Connection aConn) throws SQLException {
		ProjectTableAccessor temp = null;
		PreparedStatement ps = aConn.prepareStatement(ProjectTableAccessor.getBasicSelect() + " where title = ?");
		ps.setString(1, title);
		ResultSet rs = ps.executeQuery();
		while (rs.next()) {
			temp = new ProjectTableAccessor(rs);
		}
		rs.close();
		ps.close();
	 ...
	 * @throws SQLException
	 */
	public static List<ProjectTableAccessor> findAllProjects(Connection conn) throws SQLException {
		List<ProjectTableAccessor> projects = new ArrayList<ProjectTableAccessor>();
		PreparedStatement ps = conn.prepareStatement(ProjectTableAccessor.getBasicSelect());
		ResultSet rs = ps.executeQuery();
		while (rs.next()) {
			projects.add(new ProjectTableAccessor(rs));
		}
		rs.close();
		ps.close();
		return projects;
	...
	public static ProjectTableAccessor findFromProjectID(long projectid, Connection aConn) throws SQLException {
		ProjectTableAccessor temp = null;
