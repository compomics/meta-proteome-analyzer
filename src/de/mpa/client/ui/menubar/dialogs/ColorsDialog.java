...
import de.mpa.client.ui.resultspanel.BarChartPanel;
...
public class ColorsDialog extends JDialog {
	 ...
	 */
	private Map<Constants.UIColor, Color> colorCache;
	...
	private void initComponents() {
		...
		final JComboBox<Constants.UIColor> compCbx = new JComboBox<>(Constants.UIColor.values());
		compCbx.setRenderer(new ColorIconComboBoxRenderer() {
			@Override
			protected Color getColor(Object value) {
				return ((Constants.UIColor) value).getColor();
			}
		});
		...
		saveThemeBtn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (item instanceof UITheme) {
					...
					Map<Constants.UIColor, Color> colorMap = theme.getColorMap();
					for (Constants.UIColor uiColor : Constants.UIColor.values()) {
		...
		startColors = new Color[] {
				...
				Constants.UIColor.HORZ_BAR_CHART_HIGHLIGHTER_B_START_COLOR.getDelegateColor(),
				Constants.UIColor.HORZ_BAR_CHART_HIGHLIGHTER_C_START_COLOR.getDelegateColor() };
		endColors = new Color[] {
				Constants.UIColor.HORZ_BAR_CHART_HIGHLIGHTER_A_END_COLOR.getDelegateColor(),
				Constants.UIColor.HORZ_BAR_CHART_HIGHLIGHTER_B_END_COLOR.getDelegateColor(),
				Constants.UIColor.HORZ_BAR_CHART_HIGHLIGHTER_C_END_COLOR.getDelegateColor() };
		...
		startColors = new Color[] {
				Constants.UIColor.VERT_BAR_CHART_HIGHLIGHTER_A_START_COLOR.getDelegateColor(),
				Constants.UIColor.VERT_BAR_CHART_HIGHLIGHTER_B_START_COLOR.getDelegateColor(),
				Constants.UIColor.VERT_BAR_CHART_HIGHLIGHTER_C_START_COLOR.getDelegateColor(),
				Constants.UIColor.VERT_BAR_CHART_HIGHLIGHTER_D_START_COLOR.getDelegateColor(),
				Constants.UIColor.VERT_BAR_CHART_HIGHLIGHTER_E_START_COLOR.getDelegateColor() };
		endColors = new Color[] {
				Constants.UIColor.VERT_BAR_CHART_HIGHLIGHTER_A_END_COLOR.getDelegateColor(),
				Constants.UIColor.VERT_BAR_CHART_HIGHLIGHTER_B_END_COLOR.getDelegateColor(),
				Constants.UIColor.VERT_BAR_CHART_HIGHLIGHTER_C_END_COLOR.getDelegateColor(),
				Constants.UIColor.VERT_BAR_CHART_HIGHLIGHTER_D_END_COLOR.getDelegateColor(),
				Constants.UIColor.VERT_BAR_CHART_HIGHLIGHTER_E_END_COLOR.getDelegateColor() };
		...
		resetBtn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				picker.setColor(((Constants.UIColor) compCbx.getSelectedItem()).getColor());
			}
		});
		...
		compCbx.addItemListener(new ItemListener() {
			...
			public void itemStateChanged(ItemEvent evt) {
				picker.setColor(((Constants.UIColor) evt.getItem()).getColor());
			}
		});
		picker.addPropertyChangeListener(ColorPicker.SELECTED_COLOR_PROPERTY,
			...
			public void propertyChange(PropertyChangeEvent evt) {
				UIManager.put(((Constants.UIColor) compCbx.getSelectedItem()).getKey(),
							...
							comboBox.addActionListener(new ActionListener() {
								...
								public void actionPerformed(ActionEvent e) {
									if ("comboBoxEdited".equals(e.getActionCommand())) {
										...
										Map<Constants.UIColor, Color> newColorMap =
												new HashMap<Constants.UIColor, Color>();
										for (Entry<Constants.UIColor, Color> entry :
...
//		((JComponent) ClientFrame.getInstance().getContentPane()).revalidate();
	...
	public void setVisible(boolean b) {
		// cache colors
		this.colorCache = new HashMap<Constants.UIColor, Color>();
		for (Constants.UIColor uicolor : Constants.UIColor.values()) {
            this.colorCache.put(uicolor, uicolor.getColor());
		}
	...
	protected abstract class ColorIconComboBoxRenderer extends BasicComboBoxRenderer {
		...
		/** Color swatch icon */
		private final ColorsDialog.ColorIconComboBoxRenderer.ColorIcon icon = new ColorsDialog.ColorIconComboBoxRenderer.ColorIcon();
		...
		private class ColorIcon implements Icon {
			...
			private Color color;
			private final Insets insets = new Insets(3, 2, 0, 0);

			@Override
			public void paintIcon(Component c, Graphics g, int x, int y) {
				g.setColor(color);
				g.fillRect(insets.left, insets.top,
                        getIconWidth() - insets.right,
                        getIconHeight() - insets.bottom);
