package de.mpa.client.ui.projectpanel;
...
import de.mpa.client.ui.projectpanel.dialogs.AddExperimentDialog;
import de.mpa.client.ui.projectpanel.dialogs.AddProjectDialog;
import de.mpa.client.ui.projectpanel.dialogs.GeneralDialog;
import de.mpa.client.ui.projectpanel.dialogs.ModifyExperimentDialog;
import de.mpa.client.ui.projectpanel.dialogs.ModifyProjectDialog;
...
public class ProjectPanel extends JPanel {
	 ...
	 */
	private ArrayList<MPAProject> projects;

	/**
	 * The currently selected project.
	 */
	private MPAProject selectedProject;

	/**
	 * The currently selected experiment.
	 */
	private MPAExperiment selectedExperiment;
	...
	private void initComponents() {
		...
		// action button row
		JPanel actPnl = new JPanel(new FormLayout("r:p:g, 5dlu, r:p, 5dlu, r:p", "b:p:g"));
		...
		// @Override
		// public void actionPerformed(ActionEvent e) {
		// new FetchSpectraDialog(ClientFrame.getInstance(), "Fetch Spectra from
		// Database", getSelectedExperiment());
		// }
		// });

		// // blast results button
		// blastBtn = new JButton("Blast Results",
		// IconConstants.SEARCH_DB_ICON);
		...
		// @Override
		// public void actionPerformed(ActionEvent e) {
		// new NewBlastDialog(ClientFrame.getInstance(), "Blast Unlinked
		// Results", getSelectedExperiment());
		// }
		// });
		...
		// List<Mascothit> mascotHits = Mascothit.getHitsFromExperimentID(expID,
		// conn);
		// List<XTandemhit> xtandemHits =
		// XTandemhit.getHitsFromExperimentID(expID, conn);
		// List<Omssahit> omssaHits = Omssahit.getHitsFromExperimentID(expID,
		// conn);
		...
		// ProteinAccessor aProt =
		// ProteinAccessor.findFromID(hit.getFk_proteinid(), conn);
		...
		// Map<String, List<Long>> update_protein_map =
		// uniprotweb.find_unlinked_proteins(proteins);
	...
	private JScrollPane setupProjectTable() {
		...
		projectTbl = new JXTable(new DefaultTableModel() {
			...
			public Object getValueAt(int row, int column) {
				...
				if (value instanceof MPAProject) {
					MPAProject project = (MPAProject) value;
					switch (column) {
					case 0:
		...
		projectTbl.getSelectionModel().addListSelectionListener(new ListSelectionListener() {
			...
			public void valueChanged(ListSelectionEvent evt) {
				...
				if (selRow != -1) {
					...
					MPAProject project = (MPAProject) projectTbl.getModel().getValueAt(selRow, -1);
			...
			/**
			 * Updates states of various UI components in response to table
			 * selection.
			 */
			private void updateComponents() {
				...
				// TODO: next button should be enabled as long as a project is
				// selected
	...
	private JPanel setupProjectButtonPanel() {
		...
		addProjectBtn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent evt) {
				MPAProject project;
				project = new MPAProject("");
				AddProjectDialog dialog = new AddProjectDialog(project);
		...
		modifyProjectBtn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent evt) {
				if (selectedProject != null) {
					ModifyProjectDialog dialog = new ModifyProjectDialog(selectedProject);
					int result = dialog.showDialog();
					if (result == GeneralDialog.RESULT_SAVED) {
						// refresh table and re-select project and experiment
						MPAExperiment experiment = selectedExperiment;
						refreshProjectTable();
						refreshExperimentTable(selectedProject);
		...
		deleteProjectBtn.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent evt) {
				String confirmCode = JOptionPane.showInputDialog(ClientFrame.getInstance(),
		...
		experimentTbl = new JXTable(new DefaultTableModel() {
			...
			public Object getValueAt(int row, int column) {
				...
				if (value instanceof MPAExperiment) {
					MPAExperiment experiment = (MPAExperiment) value;
					switch (column) {
					case 0:
		...
		experimentTbl.getSelectionModel().addListSelectionListener(new ListSelectionListener() {
			...
			public void valueChanged(ListSelectionEvent evt) {
				...
				if (selRow != -1) {
					...
					selectedExperiment = (MPAExperiment) experimentTbl.getModel().getValueAt(selRow, -1);
				}
			...
			private void updateComponents() {
				...
				// check whether the selected experiment has any search results
				// associated with it
				boolean hasResult = false;
				try {
					hasResult = (selectedExperiment != null) ? selectedExperiment.hasSearchResult() : false;
				...
				skipBtn.setToolTipText(
						(hasResult) ? null : "No searches have been performed under this experiment yet.");
				nextBtn.setEnabled(true);
			}
		});
	...
	private JPanel setupExperimentButtonPanel() {
		...
		addExperimentBtn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent evt) {
				...
				experiment = new MPAExperiment("", selectedProject);
				...
				AddExperimentDialog dialog = new AddExperimentDialog(experiment);
				int res = dialog.showDialog();
				
				// check if new experiment has been created (setting a title is
				// mandatory)
		...
		modifyExperimentBtn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent evt) {
				if (selectedExperiment != null) {
					ModifyExperimentDialog dialog = new ModifyExperimentDialog(selectedExperiment);
					int result = dialog.showDialog();
					if (result == GeneralDialog.RESULT_SAVED) {
						refreshProjectTable();
						refreshExperimentTable(selectedProject);
		...
		deleteExperimentBtn.addActionListener(new ActionListener() {
			// @Override
			public void actionPerformed(ActionEvent evt) {
				int choice = JOptionPane.showConfirmDialog(clientFrame,
						"<html>Are you sure you want to delete the selected experiment?<br>Changes are irreversible.</html>",
						"Delete Experiment", JOptionPane.OK_CANCEL_OPTION, JOptionPane.QUESTION_MESSAGE);
	 ...
	 * @return the projects
	 */
	public ArrayList<MPAProject> getProjects() {
		return projects;
	}

	public MPAProject getSelectedProject() {
	 ...
	 */
	public void setSelectedProject(MPAProject project) {
		if (project != null) {
			for (int row = 0; row < projectTbl.getRowCount(); row++) {
				MPAProject p = (MPAProject) projectTbl.getValueAt(row, -1);
				if (p.equals(project)) {
	 ...
	 * 
	 * @param project
	 *            the project containing the experiments to populate the table
	 *            with
	 */
	public void refreshExperimentTable(MPAProject project) {
		TableConfig.clearTable(experimentTbl);
		
		for (MPAExperiment experiment : project.getExperiments()) {
			((DefaultTableModel) experimentTbl.getModel()).addRow(new Object[] { experiment });
		}
	}
	 ...
	 * 
	 * @param experiment
	 *            the experiment to select
	 */
	public void setSelectedExperiment(MPAExperiment experiment) {
		if (experiment != null) {
	 ...
	 */
	public MPAExperiment getSelectedExperiment() {
	...
	private class DeleteExperimentWorker extends SwingWorker {
		 ...
		 */
		private MPAExperiment experiment;
		/**
		 * The currently selected project.
		 */
		 ...
		 * @param project
		 */
		public DeleteExperimentWorker(MPAExperiment deleteexperiment, MPAProject currentProject) {
	...
	private class DeleteProjectWorker extends SwingWorker {
		 ...
		 * The currently selected project.
		 */
		private final MPAProject project;
		 ...
		 * @param project
		 */
		public DeleteProjectWorker(MPAProject currentProject) {
